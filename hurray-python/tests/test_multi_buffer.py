...
through ``__hurray__`` in one capsule, in descriptor buffer-table order.
"""
...
def test_each_call_yields_a_fresh_capsule():
    ...
    assert hurray.from_hurray(t) is not None
    assert hurray.from_hurray(t) is not None


def test_from_hurray_rejects_a_non_participant():
