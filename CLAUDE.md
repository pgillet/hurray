...
- **A pass that changes what an implementation supports updates the coverage matrix in the same pass.** A new element type, layout, quantization scheme, descriptor section, or interchange capability — in any crate — means adding its row to `website/coverage-matrix.toml` and regenerating the page with `python3 website/check-coverage-matrix.py --write` (needs an interpreter with `hurray` installed). CI fails on drift, but the rule is what stops a pass from landing red.
- **A new cookbook page is classified in the same pass.** Its Rust fences carry a tier (`rust` runs, `rust,no_run` compiles, `rust,ignore` needs a reason) and the page goes in `RUN` or `COMPILE` in `hurray-python/tests/test_cookbook_python_blocks.py`, with a reason if it is compile-only. Both checks fail on an unclassified page. See `docs/website/README.md` §7.1 — the Python tier cannot go in the fence, because `lang-tabs.js` keys tabs by language.
