# Spec-Checker Checklist

The `spec-checker` agent MUST read `README.md` at the repo root before starting any audit. The **Core Properties** section of `README.md` is the authoritative contract against which every spec section is checked. The 10 checklist categories below operationalise those 8 properties into concrete yes/no questions.

The `spec-checker` agent MUST run through every item below for each new or modified spec section before reporting findings. Items that do not apply to a given section should be noted as N/A with a brief justification.
...
- [ ] Are buffer handles described in a way compatible with the buffer-protocol.md zero-copy semantics?
...
- [ ] Is there any forward reference or back-reference to data that appears later in the stream? (MUST NOT exist.)
...
## 8. Interchange Protocol Compatibility
*(README property 8)*

- [ ] If the section defines new message types or fields, are they consistent with the framing defined in interchange.md?
- [ ] Does the section break any existing invariant in the protocol (e.g., descriptor-before-data ordering)?
- [ ] If new flags are introduced, are flag bit positions documented and reserved bits required to be 0?

## 9. RFC 2119 Correctness
...
The spec-checker agent MUST produce a structured report with the following sections:

```
## Spec-Checker Report: <section name>

### Summary
<one paragraph overall assessment>

### Checklist Results
<table: item | status (pass / fail / N/A) | finding>

### Findings
<numbered list of issues, each with severity (CRITICAL / HIGH / MEDIUM / LOW), location (file:section), and suggested resolution>
...
The agent MUST NOT edit any spec file directly. All findings go to `format-spec-writer` (editorial) or `architect` (design decisions).
