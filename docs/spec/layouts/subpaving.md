...
| `layout_tag` | `uint8` | Layout of elements within this region. MUST NOT be `0x00` or `0xFF`. |
| `_reserved` | `uint8[3]` | MUST be `0x00`. |
| `buffer_index` | `uint32` | Index into the buffer table for this region's data. |
| `byte_offset` | `uint64` | Byte offset within the referenced buffer to the start of this region's data. |

After `byte_offset`, the layout-specific fields for the region's `layout_tag` are
encoded inline (recursively if `layout_tag` is `0x04` or `0x06`).
...
3. Apply the region's layout addressing to `local` to compute the offset within the region's buffer at its `byte_offset`.
