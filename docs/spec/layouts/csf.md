...
> version. Rank-2 sparse matrices are served by CSR ([CSR (Compressed Sparse Row)](csr.md)) and CSC
> ([CSC (Compressed Sparse Column)](csc.md)), whose dense outer pointer array is more compact and more
> interop-canonical. COO ([COO (Coordinate)](coo.md)) (which supports any rank) and CSR/CSC are
...
tensors, writers SHOULD prefer CSR ([CSR (Compressed Sparse Row)](csr.md)) or CSC ([CSC (Compressed Sparse Column)](csc.md)), whose
dense outer pointer array is more compact and more interop-canonical; for rank-1 and
rank-2 coordinate lists, writers SHOULD prefer COO ([COO (Coordinate)](coo.md)). CSF MUST NOT be
used below rank 3.
