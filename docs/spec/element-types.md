...
> **[OQ-3]:** Should the private extension range (`0xF0` -- `0xFE`) require implementations to include a type descriptor (name, bit width) in the tensor metadata so that readers can at least compute buffer sizes for unknown types? This would improve interoperability of extensions at the cost of metadata complexity.
