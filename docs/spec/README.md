...
- Align the Tier 1 element type vocabulary with the Python Array API Standard, enabling zero-copy interoperability without dtype translation for standard numeric types. See [Python Bindings](../impl/python-bindings.md) for binding-level requirements.
...
| [Per-Tensor Affine](quantization/per-tensor-affine.md) | Per-tensor affine quantization — scheme tag `0x01` (Tier 1) |
| [Per-Channel Affine](quantization/per-channel-affine.md) | Per-channel (per-axis) affine quantization — scheme tag `0x02` (Tier 1) |
| [Per-Block Affine](quantization/per-block-affine.md) | Per-block affine quantization — scheme tag `0x03` (Tier 1) |
| [NF4 (NormalFloat4)](quantization/nf4.md) | NF4 (NormalFloat4) block quantization — scheme tag `0x04` (Tier 2) |
| [MXFP (OCP Microscaling)](quantization/mxfp.md) | MXFP (OCP Microscaling) block quantization — scheme tag `0x05` (Tier 2) |
| [Memory Layout](memory-layout.md) | Layout taxonomy, common fields, element address computation, alignment, sharding, buffer table |
| [Row-Major (C Order)](layouts/row-major.md) | Row-major (C order) layout — tag `0x01` |
| [Column-Major (Fortran Order)](layouts/column-major.md) | Column-major (Fortran order) layout — tag `0x02` |
| [Strided](layouts/strided.md) | Strided layout with negative/zero stride support — tag `0x03` |
| [Tiled / Blocked](layouts/tiled.md) | Tiled / blocked layout with recursive nesting — tag `0x04` |
| [Morton (Z-Order Curve)](layouts/morton.md) | Morton (Z-order curve) layout — tag `0x05` |
| [COO (Coordinate)](layouts/coo.md) | COO (Coordinate) sparse layout — tag `0x06` |
| [CSR (Compressed Sparse Row)](layouts/csr.md) | CSR (Compressed Sparse Row) sparse layout — tag `0x07` |
| [CSC (Compressed Sparse Column)](layouts/csc.md) | CSC (Compressed Sparse Column) sparse layout — tag `0x08` (also known as CCS) |
| [CSF (Compressed Sparse Fiber)](layouts/csf.md) | CSF (Compressed Sparse Fiber) sparse layout — tag `0x09` |
| [Block-Paged](layouts/block-paged.md) | Block-paged (PagedAttention KV cache) layout — tag `0x0A` |
| [Composite / Virtual Tensor](layouts/composite.md) | Composite / Virtual tensor (head + members) — tag `0x0B` |
| [Hilbert Curve](layouts/hilbert.md) | Hilbert curve layout — tag `0x40` |
